...
const tallyVotes = (roomId) => {
    ...
    // Count votes, or assign votes to a random player if no one voted
    const votesToTally = votedPlayers.length > 0 ? votedPlayers : [allPlayers[Math.floor(Math.random() * allPlayers.length)]];
    
    votesToTally.forEach(vote => { voteCounts[vote] = (voteCounts[vote] || 0) + 1; });
...
const handleTimeout = (roomId, phase) => {
    ...
    if (phase === 'hint' && room.gameState === 'playing') {
        ...
        if(currentPlayer) room.hints.push({ player: currentPlayer, type: 'text', content: '' }); // Submit empty hint
...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ playerName, roomId }) => {
    if (!rooms[roomId]) return socket.emit('error', { message: '방을 찾을 수 없습니다.' });
    if (rooms[roomId].gameState !== 'waiting') return socket.emit('error', { message: '이미 시작된 게임입니다.' });
    const player = createPlayer(playerName);
  ...
  socket.on('reconnectPlayer', ({ persistentId, roomId }) => {
    ...
    if (player) {
        ...
        socket.emit('error', { message: '재접속에 실패했습니다. 플레이어를 찾을 수 없습니다.' });
    }
  });
  ...
  socket.on('startGame', ({ roomId }) => {
    ...
    if (room.players.length < 2) return socket.emit('error', { message: '최소 2명의 플레이어가 필요합니다.' });
    ...
    room.players.forEach(player => {
        ...
        if (room.gameMode === 'fool') {
            playerSocket.emit('gameStarted', { role: 'Citizen', category: currentCategory, word: isLiar ? liarWord : citizenWord });
        } else {
            playerSocket.emit('gameStarted', { role: isLiar ? 'Liar' : 'Citizen', category: currentCategory, word: isLiar ? null : citizenWord });
        }
    });
    startTimer(roomId, HINT_TIMER_DURATION, () => handleTimeout(roomId, 'hint'));
    io.to(roomId).emit('updateRoom', room);
  ...
  socket.on('restartGame', ({ roomId }) => {
    ...
    rooms[roomId] = { ...room, ...resetRoundState(room), ...getDefaultRoomSettings() };
    io.to(roomId).emit('updateRoom', rooms[roomId]);
  });
